...
subtest 'Creating API client' => sub {
    my @tests = (
        [{id => 1}, qr/^Missing required argument: api_client/],
        [{api_clien => 1}, qr/^Missing required argument: id/],
    );
...
subtest 'Api Methods tests' => sub {
    ...
    subtest 'upsert' => sub {
        ...
        is_deeply $response->{data}, {email => 'user@example.com', created_at => $time, some => 'data'}, 'Data is correct';
    };
